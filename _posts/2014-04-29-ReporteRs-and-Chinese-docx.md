...
{% highlight text %}
##  [1] "a0" "1"  "20" "3"  "4"  "5"  "a4" "a5" "a6" "a7" "a8" "a9" "a"  "2" 
## [15] "aa" "ac" "ad" "ae" "10"
{% endhighlight %}
...
如果了解Word的使用并且经常使用其样式功能的人一定会会修改内建样式或经常创建自己需要的样式名称。关于前面提到的问题，这里有两件事要放在一起说一下：

- 对于个别应用，可以创建自己的样式来应用，但是Word的内建样式还是有很多重要功能的，并不是创建自己的样式可以完全替代的，所以理论上来说，最好是使用内建样式。
...
因此，简单来说就是既要尽量使用内建样式，又要无法更改样式名称。

### 暂时的解决方案

关于这个问题我已经向作者[提出][myissue]，看看作者能否有什么好办法改善一下这个情况，因为我们真的没办法在那里瞎猜。
...
上面提到的对标题进行重新声明，可以参考[项目主页][name]上来做:


{% highlight r %}
doc = declareTitlesStyles(doc, stylenames = c("1", "2", "3", "4", "5", "6", 
    "7", "8", "9"))
{% endhighlight %}
...
得到的结果就是如下图所示了，可以看到`styles`给出的样式实际对应的样式名称，然后根据这种方法做好自己的模板。应当说这样的方法有利于制作模板和自动化报告的程序，*磨刀不误砍柴工*，否则就有点儿费事儿，但愿作者能做些修改。
